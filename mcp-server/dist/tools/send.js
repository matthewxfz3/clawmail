...
function validateAddressList(addresses, fieldName) {
...
async function jmapRequest(accountId, calls) {
...
async function getMailboxId(accountId, name) {
...
// Tool: send_email
// ---------------------------------------------------------------------------
export async function toolSendEmail(params) {
    ...
    if (domain.toLowerCase() !== config.domain.toLowerCase()) {
        throw new Error(`from_account must belong to the configured domain "${config.domain}", got "${domain}"`);
    }
    // --- Validate recipients ---
    ...
    const createResult = createResponse[1];
    ...
    const submitResult = submitResponse[1];
