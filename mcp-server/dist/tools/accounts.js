...
import { createAccount, deleteAccount, listAccounts, accountExists, } from "../clients/stalwart-mgmt.js";
// ---------------------------------------------------------------------------
// Validation
// ---------------------------------------------------------------------------
/**
...
function validateLocalPart(localPart) {
    ...
    if (localPart.length > 64) {
        throw new Error(`local_part must be at most 64 characters (got ${localPart.length})`);
    }
    if (!LOCAL_PART_RE.test(localPart)) {
        throw new Error("local_part must contain only alphanumeric characters, dots, or hyphens, " +
...
// Tool: create_account
// ---------------------------------------------------------------------------
export async function toolCreateAccount(localPart) {
...
// Tool: delete_account
// ---------------------------------------------------------------------------
export async function toolDeleteAccount(localPart) {
    validateLocalPart(localPart);
    const email = `${localPart}@${config.domain}`;
    if (!(await accountExists(localPart))) {
...
// Tool: list_accounts
// ---------------------------------------------------------------------------
export async function toolListAccounts() {
